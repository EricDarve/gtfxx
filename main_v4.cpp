...
TEST(gtfxx, UPCXX) {
    ...
    LOG("[main] n_rank " << n_rank << " my_rank " << my_rank << " dest " << dest);
    ...
    map_comm("send").set_pack([=, &data](int3 &global_comm_idx, Task *tsk) -> int {
        ...
        LOG("[send] from thread " << i << " rank " << my_rank <<
            " global index " << global_comm_idx[0] << " to rank " << dest);
...
//                .to_rank(dest)
//                .on_receive([=, &data](int3 global_comm_idx, int d_) {
//                    LOG("[on_receive] with " << global_comm_idx[0]);
//                    /* Argument types must match send() */
//                    assert(global_comm_idx[0] >= 0 && global_comm_idx[0] < n_thread * n_rank);
//                    assert(d_ == 1);
//                    assert(data[global_comm_idx[0]] == -1);
//                    data[global_comm_idx[0]] = d_;
...
//                });
        ...
        LOG("[send] DONE from thread " << i << " rank " << my_rank <<
                                  " global index " << global_comm_idx[0] << " to rank " << dest);
    ...
    map_comm("send").set_finalize([](int3 &global_comm_idx) {
        LOG("[set_finalize] with " << global_comm_idx[0]);
        map_task("reduce").decrement_wait_count({0, 0, 0});
        {
            auto t_ = &(map_task("reduce").task_grid(0,0,0));
            int wait_count = t_->m_wait_count.load();
            LOG("wait_count is " << wait_count <<
                " on rank " << upcxx::rank_me());
        }
        LOG("[set_finalize] done decrement reduce on rank " << upcxx::rank_me());
    });
